...
### 11. Cinema Gold & Indigo Theme Rule (Completed)
...
### 12. Itch.io Iframe Link Hiding (Completed)
...
### 13. Layout Shift & Dynamic Script Loading Fixes (Completed)
...
### 14. Anchor Layout to Top (Completed)
...
### 15. Autofocus Scroll Jump Fix (Completed)
...
- **Solution:**
  - Updated the autofocus code block in `loadLevel()` inside [app.js](file:///c:/Users/iwand/OneDrive/Documents/GitHub/PunFiction/boxofficeblunders/app.js) to skip auto-focusing on startup when running inside iframes or portal environments.
  - Implemented the modern `preventScroll: true` focus parameter option for direct desktop plays.

### 16. Referrer Check Refinement (Completed)
...
### 17. Dynamic Play Random Challenge & Completed Message (Completed)
- **Problem:** When players finished today's daily puzzle on the main website, only the "Share Score" button was displayed, leaving no clear next step.
- **Solution:**
...
### 18. Stacked Victory Actions (Completed)
...
### 19. Relocated Share Score Button (Completed)
...
### 20. Footer Navigation Cleanups (Completed)
...
### 21. Share Score Button Inset Trim (Completed)
...
### 22. Play Random green theme (Completed)
...
### 23. Boxy Solved-Box Share Score button (Completed)
...
### 24. Purple Color Revert (Completed)
...
### 25. Playlin Badge Relocation (Completed)
...
### 26. Fixed Broken Poster Links (Completed)
- **Problem:** When playing historical challenges (such as #029 "Clean Girls"), the movie poster illustration returned a 404 broken image link.
- **Solution:**
...
### 27. Automated Pre-Commit Hook (Completed)
- **Problem:** Manually tracking which poster PNG files correspond to database additions is error-prone.
- **Solution:**
  - Created [backend/verify_posters_hook.py](file:///c:/Users/iwand/OneDrive/Documents/GitHub/PunFiction/backend/verify_posters_hook.py) to check staged changes.
  - Created `.git/hooks/pre-commit` script to execute the Python script on every commit event.

### 28. CrazyGames SDK Removal (Completed)
- **Problem:** The game has been removed from CrazyGames, so we no longer need the CrazyGames portal integrations, rewarded/midgame video ads requests, compliance styling blocks, and terms consent links.
- **Solution:**
  - Removed the CrazyGames SDK v3 script tag completely from the `<head>` of [index.html](file:///c:/Users/iwand/OneDrive/Documents/GitHub/PunFiction/boxofficeblunders/index.html).
  - Cleaned up the `isCrazyGames` and `crazySDK` global variables, the `initCrazyGamesSDK()` function, and environment checking blocks inside `window.onload` in [app.js](file:///c:/Users/iwand/OneDrive/Documents/GitHub/PunFiction/boxofficeblunders/app.js).
  - Removed CrazyGames rewarded ad request logic from the Hint 4 (Vowel Rush) click handler in [app.js](file:///c:/Users/iwand/OneDrive/Documents/GitHub/PunFiction/boxofficeblunders/app.js), restoring standard Google Publisher Tag ads routing.
  - Removed CrazyGames `gameplayStart()` and `gameplayStop()` lifecycle calls from level load and victory transitions in [app.js](file:///c:/Users/iwand/OneDrive/Documents/GitHub/PunFiction/boxofficeblunders/app.js).
  - Removed CrazyGames midgame interstitial ad request from the victory sequence, switching screen to the victory stats display immediately.
  - Restored Google Publisher Tag (GPT) setup bypass exclusions.
  - Removed the terms and privacy consent block (`.settings-footer-tos`) from the bottom of the Settings modal in [index.html](file:///c:/Users/iwand/OneDrive/Documents/GitHub/PunFiction/boxofficeblunders/index.html).
  - Removed CrazyGames-specific environment layout overrides (`body.crazygames-env` hiding rules) from [style.css](file:///c:/Users/iwand/OneDrive/Documents/GitHub/PunFiction/boxofficeblunders/style.css), keeping only the active Itch.io sandbox overrides.
  - Deleted the local distribution package `punfiction_crazygames.zip` from the `boxofficeblunders/` workspace directory.
